...
import ParallelCoordinate.model.Brush;
...
class RenderThread extends Thread {
     ...
     *
     * @param ui The BasicParallelDisplayUI this thread should render.
     */
     ...
     *
     * @param comp The ParallelDisplay component this thread renders.
     */
     ...
     *
     * @param brushMode A boolean indicating whether this thread renders the brush display.
     */
     ...
     *
     * @param quality A boolean indicating whether this thread should render optimized for
     * quality (true) or speed (false).
     * @param progressive A boolean indicating whether this thread should render in progressive
     * mode. If set to true, the image is rendered in two passes: the first pass optimized
     * for speed (preview), the socond pass optimized for quality. If set to false, the
    ...
    /**
     * Sets the region that this thread should render. This method invalidates the given
     * region and tells the renderer to include it in the next rendering loop. If there is
     * another invalidated region, the region rendered in the next pass is expanded to include
     * all areas that are now invalidated.
     *
     * @param startAxis The start axis of the region to be rendered.
     * @param stopAxis The end axis of the region to be rendered.
     */
    synchronized void setRegion(int startAxis, int stopAxis){
        ...
        //this.ids = ids.clone();
        //System.out.println("RenderThread: setting repaint axes: " + this.startAxis + ", " + this.stopAxis);
    }

    /**
     * Sets the style how records are painted.
     *
     * @param stroke A Stroke object containing information about line style, width etc.
     * @param color The Color to be used to paint the records.
     */
    ...
    /**
     * Returns the last rendered image this thread has finished rendering. Note that the image contains
     * only those pixels between the last startAxis and stopAxis. All other pixels are transparent.
     *
    ...
    /**
     * The main method of this thread. Runs an infinite loop, waiting for rendering data and
     * rendering the data in one or two passes (in progressive mode).
     * In low quality mode and the first pass of progressive mode, the thread ignores
     * interruptions received during rendering. In second pass progressive mode rendering
     * can be interrupted after each record, for example if new data is available or some area
     * has been invalidated.
     *
     * Do not interfere with the thread directly, use render() to signalize that there is something
     * new to render.
     */
    public void run(){
        while(true){
            synchronized(this){
                ...
                do {
                    try {
                        ...
                        System.out.println("RenderThread: interruptedExcpetion.");
            ...
            while (comp != null && progress){
                ...
                if (progressive && quality){
                    ...
                    //System.out.println("RenderThread: starting progressive paint...");
                    modestr = "[quality]";
    ...
    /**
     * Starts the rendering. The region to be rendered must be set with setRegion(). If
     * there is a high quality rendering in progress it is interrupted.
     */
     ...
     */
    private void setupRendering(Graphics2D g2, boolean quality, Stroke stroke, Color color){
